...
class Paginator(ui.View):
    ...
    def create_embed(self, interaction:discord.Interaction, data):
        ...
        try:
            embeds[0].set_author(name=f"{interaction.user.name}'s Liked Images", icon_url=interaction.user.avatar.url)
        except:
            embeds[0].set_author(name=f"{interaction.user.name}'s Liked Images", icon_url=interaction.user.default_avatar.url)
        embeds[0].set_footer(text=f"Page {self.current_page}/{math.ceil(len(self.data) / self.sep)}")
        embeds[0].add_field(name="The Prompts for the Images you liked are below.", value=f"```{prompts_}```", inline=False)
